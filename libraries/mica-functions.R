...
initMica <- function( micaJavaPath ) {
	...
	if (sum(file.access(paste(micaJavaPath ,file.sep ,"mica.jar",sep=""),4)) == 0 )  {
	...
	for( depJar in depJars ) {
		if (sum(file.access(paste(micaJavaPath,depJar,sep=file.sep),4)) != 0 ) stop(paste("given micaJavaPath='",micaJavaPath,"' does not contain needed jar file '",depJar,"'",sep=""));
	}
	...
	if (allInOneJarAvailable) {
		rJava::.jaddClassPath(paste(micaJavaPath, file.sep, "mica.jar",sep=""));
	} else {
		rJava::.jaddClassPath(micaJavaPath)
	}
	for( depJar in depJars ) {
		rJava::.jaddClassPath(paste(micaJavaPath,depJar,sep=file.sep));
	}
...
interpolateCurves <- function( x, y, samples ) {
	...
	for (c in 1:ncol(x)) {
	  ...
	  if (length(xC) == 0) next;
	  yC <- as.vector(na.omit(y[,c]));
	  sampled <-  approx( xC, yC, n = samples );
		xNew[,c] <- sampled$x;
		yNew[,c] <- sampled$y;
	}
...
#' 
#' @export
#' @author Alexander Mattheis (copied Martin Radens function for interpolation)
getSlopeOfCurves <- function( x, y ) {
  ...
  yNew <- matrix(NA, ncol=ncol(x), nrow=nrow(x));
  ...
  for (c in 1:ncol(x)) {
    ...
    if (length(xC) == 0) next;
    yC <- as.vector(na.omit(y[,c]));
    sampled <- getSlope( xC, yC );
    
    # to add NAs at the end
    length(sampled$x) <- nrow(x);  
    length(sampled$y) <- nrow(x);
...
#########################################################
#' Returns the slope for the given coordinates.
#'
#' @param x the x-values of the curves' points (data.frame(vector(double)) or matrix, nrow >= 3)
#' @param y the y-values of the curves' points (data.fram(vector(double)) or matrix, nrow >= 3)
#' @param samples the number of equidistant points to be interpolated (integer > 3)
...
getSlope <- function(x, y) {
  ...
  yNew <- x;
  for (i in 1:length(yNew)) {
    yNew[i] <- rJava::.jcall( jCurve, "D", "getSlope", x[i]);
    #print(yNew[i]);
  }
  
  return(list(x=x,y=yNew));
}
